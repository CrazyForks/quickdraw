...
**tldraw**, **Excalidraw**, and **Quickdraw**. All three will put a competent
infinite canvas in your app. They differ sharply on licensing, weight, and
what stacks they serve. (Disclosure: this is Quickdraw's blog — so we'll be
concrete and source our claims, and we link both competitors throughout.)

## Licensing — the fork in the road

**tldraw** is source-available, not open source. Production use of the 3.x SDK
requires either the "Made with tldraw" watermark on the canvas or a
...
that's often fine. For a side project, an internal tool with procurement
rules, or a startup counting dollars, it's a real cost.

**Excalidraw** is MIT. Free for anything, forever.

**Quickdraw** is MIT. Same deal.
...
Both tldraw and Excalidraw are React components at their core. Quickdraw's
engine is framework-free ESM (the React wrapper is a thin binding), which is
what makes Vue/Svelte/vanilla embeds and the
[React Native package](/blog/react-native-whiteboard-drawing/) possible.

## Weight and dependencies

- **tldraw**: a full editor framework — powerful, correspondingly large, with
  a substantial dependency tree.
- **Excalidraw**: a big component too; you're embedding the Excalidraw app
  experience, assets and all.
...
The flip side of weight is capability: tldraw's shape/tool plugin system is
the richest of the three, and Excalidraw ships years of accumulated features
(libraries of components, rich export options, localization). Quickdraw
deliberately ships a complete-but-focused whiteboard; its
...
loved, and hard to restyle away. tldraw looks like a polished design tool.
Quickdraw sits between: crisp UI chrome, hand-drawn wobble in the shapes,
pressure-tapered ink, and twelve named colors that re-resolve when the board
...
- **Quickdraw** builds sync into the data model — every change emits a
  ...
  There's no first-party server yet; the relay is ~15 lines of WebSocket code.

## Maturity

Be honest about this one: tldraw and Excalidraw have enormous communities,
years of production hardening, and large test surfaces. Quickdraw is new —
...
everything here is MIT on GitHub.

## Recommendations

- **Funded product, budget for the license, want the richest editor
  framework:** tldraw.
- **Want maximum battle-testing with MIT, React-only is fine, the Excalidraw
  aesthetic fits:** Excalidraw.
- **Want MIT + lightweight embed, plain-JS/Vue/Svelte support, or React
